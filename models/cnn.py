...
class ResBlock1d(nn.Module):
  #1-D residual block: Conv -> BN -> ReLU -> Conv -> BN + skip. Keeps spatial dimension the same (no downsampling here; pooling is done explicitly between blocks).
...
class ChannelAttention(nn.Module):
  ...
  Particularly useful because spoofing is centred on GPS L1 (1575 MHz)
